...
async function parseDemonstrativoFundeb(bufferPdf) {
  const dadosPdf = await (pdf.default || pdf)(bufferPdf);
  ...
  for (const [numeroBase, config] of Object.entries(MAPA_LIMITES)) {
    ...
    let percentual;
    if (numeroBase === '14') {
      // Item 14 é um TETO: o percentual da linha base é % das receitas,
      // não % do teto. Calculamos sempre a partir de exigido/aplicado.
      percentual = (exigido && aplicado != null)
        ? Math.round((aplicado / exigido) * 10000) / 100
        : null;
    } else {
      percentual = linhaBase && linhaBase.valores.length
