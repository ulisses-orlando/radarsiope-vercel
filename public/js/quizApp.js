...
(function () {
    ...
    // Estrutura: { nid, uid, qIndex, answers: [], score: 0, finished: boolean }
    let _state = null;
    ...
    function init(newsletter, user) {
        ...
        if (!newsletter || !newsletter.quiz || !newsletter.quiz.ativo) return;
    ...
    function processarResposta(idxSelecionado) {
        const perguntas = _config.perguntas;
        const atual = perguntas[_state.qIndex];
        const acertou = (idxSelecionado === parseInt(atual.correta)); // Garante number

        if (acertou) _state.score++;

        // Salva na memória
        _state.answers.push({
            qId: atual.id, // ou index
    ...
    async function finalizarQuiz() {
        ...
        // 2. Salva no Backend
        await salvarNoBackend(pontuacao, aprovado);
    ...
    async function salvarNoBackend(pontuacao, aprovado) {
        const payload = {
            uid: _state.uid,
            newsletter_id: _state.nid,
            pontuacao: pontuacao,
            aprovado: aprovado,
            respostas: _state.answers.map(a => ({ qId: a.qId, idx: a.selecionada }))
        };

        try {
            // Chama o endpoint que criaremos no pagamentoMP.js
            ...
            // Marca localmente como concluído
            marcarConcluidoLocal(_state.uid, _state.nid);
            
            // Dispara evento para atualizar badges (se houver listener)
            window.dispatchEvent(new CustomEvent('rs:quizConcluido', { detail: { nid: _state.nid } }));
    ...
    function marcarConcluidoLocal(uid, nid) {
        try {
    ...
    // Expõe a função de inicialização para o script principal chamar
