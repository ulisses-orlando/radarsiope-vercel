...
(function () {
  ...
  const NW   = { root: 140, l1: 175, l2: 210 }; // larguras por nível
  const NH   = 44;    // altura de cada nó
  const HGAP = 68;    // gap horizontal entre níveis
  const VGAP = 12;    // gap vertical entre irmãos
  const PAD  = 48;    // padding da tela
  const BR   = 10;    // border-radius dos nós

  const MIN_SCALE = 0.2;
  const MAX_SCALE = 2.5;

  // Paleta derivada da identidade Radar — funciona em dark theme
  const PALETA = [
    '#60A5FA', // azul
    '#34D399', // verde
    '#A78BFA', // roxo
  ...
  // ── Estado interno ──────────────────────────────────────────────────────
  let _dados     = null;        // newsletter.mapa_mental
  let _expanded  = new Set();   // IDs dos nós de nível-1 expandidos
  let _tf        = { x: 0, y: 0, scale: 1 };
  let _svgEl     = null;
  let _svgG      = null;
  let _layout    = null;        // último layout calculado
  ...
  /**
   * Ponto de entrada. Chamado por renderMidia() em verNewsletterComToken.js.
   * @param {Object} newsletter - documento completo da newsletter
   * @param {Object} acesso     - objeto de controle de acesso do assinante
   */
  ...
  function _renderizarCard(newsletter) {
    // Ajuste o seletor conforme o container de mídias do projeto
  ...
  function _abrirModal() {
    ...
    // Renderiza e centra após o DOM estar pronto
  ...
  function _toggleExpandirTodos() {
    ...
    if (todosExp) {
      ...
      filhos.forEach(n => _expanded.add(n.id));
    }

    _renderizarGrafo();
    // Recentraliza após expandir pois o canvas muda de tamanho
  ...
  /**
   * Calcula as posições absolutas (x, y) de cada nó no canvas SVG.
   * Retorna: { pos: { [nodeId]: NodePos }, canvasW, canvasH }
   */
  function _calcularLayout() {
    ...
    // Altura que cada nó de nível-1 ocupa no eixo vertical
    // (própria altura, ou altura dos filhos se expandido)
  ...
  function _renderizarGrafo() {
    ...
    filhos.forEach(node => {
      const p = pos[node.id];
      _drawCurve(
        rootP.x + rootP.w, rootP.y + rootP.h / 2,
        p.x,               p.y + p.h / 2,
        p.cor, 2, 0.5
      );
    });
    ...
    filhos.forEach(node => {
      ...
      node.filhos.forEach(child => {
        ...
        if (!c) return;
        _drawCurve(
          parent.x + parent.w, parent.y + parent.h / 2,
          c.x,                 c.y + c.h / 2,
          parent.cor, 1.5, 0.4
        );
      });
    ...
    // ── 3. Nós (desenhados após conexões para ficarem na frente) ──────────
    Object.values(pos).forEach(_drawNode);
  }

  /** Desenha curva bézier cúbica entre dois pontos */
  function _drawCurve(x1, y1, x2, y2, cor, strokeW, opacity) {
    const mx = (x1 + x2) / 2;
    const path = _svgEl_('path');
    path.setAttribute('d',
  ...
  /** Desenha um nó completo (rect + texto + indicador) */
  function _drawNode(p) {
    ...
    if (p.tipo === 'root') {
      ...
      // Faixa colorida na borda esquerda
      const faixa = _svgEl_('rect');
    ...
    if (linhas.length === 1) {
      ...
      // 2 linhas: centradas verticalmente no nó
    ...
    // ── Evento de clique/tap em nível 1 ──────────────────────────────────
    if (p.tipo === 'l1') {
  ...
  /** Quebra texto em no máximo 2 linhas com base na largura disponível */
  function _wrapText(texto, maxW, fontSize) {
    // Estimativa: char width ≈ fontSize * 0.55
    const maxChars = Math.floor(maxW / (fontSize * 0.56));
    if (!texto || texto.length <= maxChars) return [texto];

    // Tenta quebrar na última palavra antes do meio
    ...
    return [
      texto.slice(0, quebra).trim(),
      texto.slice(quebra).trim(),
    ];
  }

  // ══════════════════════════════════════════════════════════════════════
  // INTERAÇÃO — PAN, ZOOM, TAP
  // ══════════════════════════════════════════════════════════════════════

  // Flag para distinguir drag de tap
  ...
  function _centralizarVista(animado = false) {
    ...
    // Nunca amplia além de 1× no reset para preservar legibilidade
  ...
  /** Cria elemento SVG no namespace correto */
  function _svgEl_(tag) {
    return document.createElementNS('http://www.w3.org/2000/svg', tag);
  }

  /** Escapa HTML para evitar XSS em textos do Firestore */
  function _esc(str) {
    return String(str)
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
  ...
  function _injetarCSS() {
    ...
    s.textContent = `
      ...
      .rs-mm-badge {
        background: rgba(10,61,98,0.6); color: #60A5FA;
        font-size: 10px; font-weight: 700; letter-spacing: .08em;
      ...
      #rs-mm-btn-reset:hover,
      #rs-mm-btn-fechar:hover,
      #rs-mm-btn-expandir:hover {
      ...
      /* Transição suave nas curvas ao expandir/recolher */
      .rs-mm-curve {
      ...
      @media (max-width: 480px) {
        .rs-mm-card { flex-wrap: wrap; }
        .rs-mm-card-btn { width: 100%; text-align: center; }
        .rs-mm-hint span:not(.rs-mm-sep):not(:first-child),
        .rs-mm-sep:not(:first-of-type) { display: none; }
      }
    `;
