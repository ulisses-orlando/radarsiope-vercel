/* ==========================================================================
   mapaMental.js — Módulo de Mapa Mental Interativo
   ...
   v2 — Popover de detalhe em nós de nível 2
   ========================================================================== */
(function () {
  'use strict';

  // ── Constantes de Layout ────────────────────────────────────────────────
  const NW  = { root: 140, l1: 175, l2: 214 };
  const NH  = 44;
  const HGAP = 68;
  const VGAP = 12;
  const PAD  = 48;
  ...
  let _popoverId    = null;   // nodeId do popover aberto (null = fechado)
  ...
  function _renderizarCard() {
    ...
    card.innerHTML =
      ...
      '<div class="rs-mm-card-info">' +
        ...
        '<span class="rs-mm-card-titulo">' + _esc(_dados.titulo || 'Mapa da edicao') + '</span>' +
      '</div>' +
      '<button class="rs-mm-card-btn" type="button">Explorar &rarr;</button>';
  ...
  function _abrirModal() {
    ...
    overlay.innerHTML =
      '<div class="rs-mm-modal">' +
        ...
        '<div class="rs-mm-hint">' +
          '<span>Toque nos t&oacute;picos para expandir</span>' +
          '<span class="rs-mm-sep">&middot;</span>' +
          '<span>Subtópicos com 💡 têm detalhes</span>' +
          '<span class="rs-mm-sep">&middot;</span>' +
          '<span>Arraste e use pinça para navegar</span>' +
        '</div>' +
      '</div>';
    ...
    // Fecha popover ao clicar no fundo do SVG
    _svgEl.addEventListener('click', e => {
      if (e.target === _svgEl || e.target.id === 'rs-mm-bg') _fecharPopover();
    });
  }
  ...
  function _calcularLayout() {
    ...
    filhos.forEach((node, idx) => {
      ...
      pos[node.id] = {
        ...
        tipo: 'l1', texto: node.texto, cor, nodeId: node.id,
        temFilhos: !!(node.filhos?.length),
      };
      ...
      if (_expanded.has(node.id) && node.filhos?.length) {
        node.filhos.forEach((child, cidx) => {
          pos[child.id] = {
            ...
            nodeId: child.id,
            detalhe: child.detalhe || null,   // campo opcional
          };
        });
      }
  ...
  function _renderizarGrafo() {
    ...
    // Conexoes raiz -> nivel 1
    const rootP = pos['root'];
    filhos.forEach(node => {
      const p = pos[node.id];
      _drawCurve(rootP.x + rootP.w, rootP.y + rootP.h / 2,
                 p.x,               p.y + p.h / 2,
                 p.cor, 2, 0.5);
    });

    // Conexoes nivel 1 -> nivel 2
    filhos.forEach(node => {
      ...
      node.filhos.forEach(child => {
        ...
        if (!c) return;
        _drawCurve(parent.x + parent.w, parent.y + parent.h / 2,
                   c.x,                 c.y + c.h / 2,
                   parent.cor, 1.5, 0.4);
      });
    });
  ...
  function _drawNode(p) {
    ...
    if (p.tipo === 'root') {
      ...
      // Nivel 2
      const isActive = (_popoverId === p.nodeId);
      rect.setAttribute('fill',
        isActive ? p.cor + '28' : p.cor + '12');
      rect.setAttribute('stroke',
        isActive ? p.cor + 'cc' : p.cor + '44');
      rect.setAttribute('stroke-width', isActive ? '1.5' : '1');

      // Indicador de detalhe (circulo luminoso no canto direito)
      if (p.detalhe) {
        g.style.cursor = 'pointer';

        const dot = svgEl('circle');
        dot.setAttribute('cx',   p.x + p.w - 10);
        dot.setAttribute('cy',   p.y + 10);
        dot.setAttribute('r',    5);
        dot.setAttribute('fill', p.cor);
        dot.setAttribute('opacity', '0.9');
        g.appendChild(dot);

        // Mini icone "i" dentro do circulo
        const dotLabel = svgEl('text');
        dotLabel.setAttribute('x',           p.x + p.w - 10);
        dotLabel.setAttribute('y',           p.y + 14);
        dotLabel.setAttribute('font-size',   '7');
        dotLabel.setAttribute('font-weight', '900');
        dotLabel.setAttribute('fill',        '#fff');
        dotLabel.setAttribute('text-anchor', 'middle');
        dotLabel.setAttribute('font-family', 'system-ui, sans-serif');
        dotLabel.textContent = 'i';
    ...
    const reservaDirL2 = (p.tipo === 'l2' && p.detalhe) ? 20 : paddingX;
    const maxTextW   = p.w - paddingX - (p.tipo === 'l1' ? 28 : reservaDirL2);
    ...
    } else if (p.tipo === 'l2' && p.detalhe) {
      g.addEventListener('click', e => {
        e.stopPropagation();
        if (!_wasDragging) _togglePopover(p);
      });
    }
  }
  ...
  // POPOVER DE DETALHE
  // ══════════════════════════════════════════════════════════════════════
  function _togglePopover(p) {
    if (_popoverId === p.nodeId) {
      _fecharPopover();
      return;
    }
    _mostrarPopover(p);
  ...
  function _mostrarPopover(p) {
    _fecharPopover();
    _popoverId = p.nodeId;

    // Redesenha o no ativo com destaque
    _renderizarGrafo();
    ...
    // Coordenadas do no em relacao ao canvas-wrap
    const nodeRight  = _tf.x + (p.x + p.w) * _tf.scale;
    const nodeCenterY = _tf.y + (p.y + p.h / 2) * _tf.scale;

    // Posicao preferida: a direita do no
    let left = nodeRight + 10;
    let anchorDir = 'right';

    // Se nao cabe a direita, vai para a esquerda
    if (left + POPOVER_W > wrap.clientWidth - 8) {
      left = _tf.x + p.x * _tf.scale - POPOVER_W - 10;
      anchorDir = 'left';
    }

    // Vertical: centralizado no no, clampado na tela
    const estimatedH = 90;
    let top = nodeCenterY - estimatedH / 2;
    top = Math.max(8, Math.min(top, wrap.clientHeight - estimatedH - 8));

    const pop = document.createElement('div');
    pop.id        = 'rs-mm-popover';
    pop.className = 'rs-mm-popover rs-mm-popover--' + anchorDir;
    pop.style.cssText =
      'left:' + left + 'px;top:' + top + 'px;' +
      'border-left-color:' + p.cor + ';';

    pop.innerHTML =
      '<div class="rs-mm-pop-header">' +
        '<span class="rs-mm-pop-dot" style="background:' + p.cor + '"></span>' +
        '<span class="rs-mm-pop-label">Detalhes</span>' +
        '<button class="rs-mm-pop-fechar" type="button">&#x2715;</button>' +
      '</div>' +
      '<div class="rs-mm-pop-texto">' + _esc(p.detalhe) + '</div>';

    wrap.appendChild(pop);

    // Anima entrada
    requestAnimationFrame(() => pop.classList.add('rs-mm-popover--visible'));

    pop.querySelector('.rs-mm-pop-fechar')
      .addEventListener('click', e => { e.stopPropagation(); _fecharPopover(); });
  }

  function _fecharPopover() {
    const pop = document.getElementById('rs-mm-popover');
    if (pop) {
      pop.classList.remove('rs-mm-popover--visible');
      setTimeout(() => pop.remove(), 180);
    }
    const anteriorId = _popoverId;
    _popoverId = null;
    // Redesenha apenas se havia um no ativo (remove o destaque)
    if (anteriorId && _svgG) _renderizarGrafo();
  }

  // ══════════════════════════════════════════════════════════════════════
  // INTERACAO — PAN, ZOOM, TAP
  // ══════════════════════════════════════════════════════════════════════
  ...
  function _centralizarVista(animado) {
    const wrap = document.getElementById('rs-mm-canvas-wrap');
    if (!wrap || !_layout) return;
    const wW = wrap.clientWidth;
    const wH = wrap.clientHeight;
    const scale = Math.min((wW - 32) / _layout.canvasW, (wH - 32) / _layout.canvasH, 1);
    _tf = {
  ...
  function _applyTransform(animado) {
    if (!_svgG) return;
    if (animado) {
    ...
    _svgG.style.transform =
      'translate(' + _tf.x + 'px,' + _tf.y + 'px) scale(' + _tf.scale + ')';
  }
  ...
  function _setupInteracao() {
    ...
    _svgEl.addEventListener('touchmove', e => {
      ...
      if (e.touches.length === 1 && pinchDist0 === null) {
        ...
        _tf.scale  = Math.min(MAX_SCALE,
                       Math.max(MIN_SCALE, pinchScale0 * (dist / pinchDist0)));
        _applyTransform();
      }
    }, { passive: false });
  ...
  function _injetarCSS() {
    ...
    s.textContent = [
      ...
      // Popover
      '.rs-mm-popover{position:absolute;z-index:100;width:248px;background:#1e2d45;border:1px solid rgba(255,255,255,0.1);border-left:3px solid #60A5FA;border-radius:10px;padding:12px 14px;box-shadow:0 8px 32px rgba(0,0,0,0.5);opacity:0;transform:scale(0.92) translateY(4px);transition:opacity .18s ease,transform .18s ease;pointer-events:none}',
      '.rs-mm-popover--visible{opacity:1;transform:scale(1) translateY(0);pointer-events:auto}',
      '.rs-mm-pop-header{display:flex;align-items:center;gap:7px;margin-bottom:8px}',
      '.rs-mm-pop-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0}',
      '.rs-mm-pop-label{font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#64748b;flex:1}',
      '.rs-mm-pop-fechar{background:none;border:none;color:#475569;cursor:pointer;font-size:13px;padding:0;line-height:1;transition:color .15s}',
      '.rs-mm-pop-fechar:hover{color:#f1f5f9}',
      '.rs-mm-pop-texto{font-size:13px;line-height:1.6;color:#cbd5e1}',

      // Mobile
      '@media (max-width:480px){.rs-mm-card{flex-wrap:wrap}.rs-mm-card-btn{width:100%;text-align:center}.rs-mm-hint span:not(.rs-mm-sep):not(:first-child),.rs-mm-sep:not(:first-of-type){display:none}.rs-mm-popover{width:calc(100vw - 32px);left:16px !important}}',
    ].join('');
