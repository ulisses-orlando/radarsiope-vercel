...
async function _checarSessaoCritica() {
  // ✅ MODO PREVIEW: bypass completo da validação de sessão
  if (window._isPreviewMode === true) return true;
...
async function validarETocarAudio(url, btn) {
  ...
  // ✅ MODO PREVIEW: bypass completo, sem validação de sessão
  if (window._isPreviewMode === true) {
    ...
    // Tenta autoplay, se falhar mostra link de fallback
    try {
      await audio.play();
    } catch (e) {
      const fallback = document.createElement('div');
      fallback.style.cssText = 'font-size:11px;color:#94a3b8;text-align:center;margin-top:6px;';
      fallback.innerHTML = `<a href="${url}" target="_blank" style="color:#60A5FA;text-decoration:underline;">Clique aqui para abrir o áudio em nova aba</a>`;
      wrap.appendChild(fallback);
    }
    return;
  }

  // ⚠️ FLUXO NORMAL (assinantes/leads) — com validação de sessão
  if (await _checarSessaoCritica()) {
    const wrap = btn.parentElement;
    btn.style.display = 'none';
...
async function _executarPreview(params) {
  // ✅ Define flag global para bypass de validação de sessão
...
function _cardEdicaoAssinante(ed, isAtual, temAcesso, uid) {
  ...
  return `<div class="rs-drawer-ed-card-wrap" style="display:flex;align-items:center;gap:4px;padding:2px 4px;overflow:visible;">
    ...
    <button class="rs-drawer-ed-card ${classeAtual}" style="flex:1;margin:0;min-width:0;" onclick="navegarParaEdicao('${_esc(ed.id)}')" type="button">
      <div class="rs-drawer-ed-info">
        <div class="rs-drawer-ed-titulo">
          ${titulo}
          ${ed.formato === 'extra' ? '<span style="font-size:9px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;background:rgba(245,158,11,.15);color:#d97706;border:1px solid rgba(245,158,11,.3);border-radius:4px;padding:1px 5px;margin-left:6px;vertical-align:middle">⚡ extra</span>' : ''}
        </div>
        <div class="rs-drawer-ed-data">${data}${num ? ` · Ed. ${num}` : ''}</div>
      </div>
