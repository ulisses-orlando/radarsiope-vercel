        ...
        for (let i = 0; i < destinatarios.length; i += CHUNK_SIZE) {
            ...
            const settled = await Promise.allSettled(
                chunk.map(async (dest) => {
                    ...
                    if (tipo === 'leads') {
                        // ── Verifica se já existe um registro "pendente" para este lead/newsletter ──
                        const { data: existente, error: queryError } = await window.supabase
                            .from('leads_envios')
                            .select('id, status')
                            .eq('lead_id', idDest)
                            .eq('newsletter_id', newsletterId)
                        ...
                        if (existente) {
                            ...
                            registroEnvioId = String(existente.id);
                            _logProgresso(`♻️ Reutilizando registro pendente para ${emailDest}`, 'aviso');

                        } else {
                            // ✅ Não existe pendente: insere novo registro (permite histórico)
