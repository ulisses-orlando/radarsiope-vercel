...
if (tipoAtivo === 'sugestao_tema') {
  ...
  const placeholderTxt = quotaEsgotada ? 'Cota de sugestões esgotada para este mês.' : `Descreva o tema que gostaria que fosse abordado… você ainda tem direito a ${restantes} sugestão(ões).`;
  const textareaAttrs = quotaEsgotada ? 'disabled readonly style="opacity:0.6; cursor:not-allowed;"' : '';
  html += `<div style="margin-top:16px"><label class="rs-fc-label" for="rs-fc-txt">💡 Sugestão de tema</label><textarea id="rs-fc-txt" class="rs-fc-textarea" placeholder="${placeholderTxt}" maxlength="${MAX_CHARS}" ${textareaAttrs}></textarea><div class="rs-fc-chars" id="rs-fc-chars">0/${MAX_CHARS}</div></div><button class="rs-fc-enviar" id="rs-fc-enviar" disabled ${quotaEsgotada ? 'disabled' : ''} onclick="window._fcEnviar('sugestao_tema')">Enviar</button>`;
...
if (textarea) {
  textarea.addEventListener('input', () => { const n = textarea.value.length; chars.textContent = `${n}/${MAX_CHARS}`; chars.classList.toggle('limite', n >= MAX_CHARS); if (!btnEnv) return; if (tipoAtivo === 'sugestao_tema') { btnEnv.disabled = (n === 0) || quotaEsgotada; } else { btnEnv.disabled = (n === 0); } });
}
}
// ── Selecionar tipo ───────────────────────────────────────────────────────
...
try {
// ✅ AJUSTADO: Busca ciclos recentes e filtra pelo validador de datas/status
const ciclosSnap = await window.db.collection('ciclos_votacao').orderBy('inicio_votacao', 'desc').limit(3).get();
let cicloAtivo = null, cicloTitulo = '';
for (const doc of ciclosSnap.docs) {
  const data = { id: doc.id, ...doc.data() };
  const val = _validarJanelaCiclo(data, 'votacao');
  if (val.valido) { cicloAtivo = data; cicloTitulo = data.titulo; break; }
}

const labelSep = cicloTitulo ? `🗳️ Votação atual: ${cicloTitulo}` : '🗳️ Votação Atual';
if (!cicloAtivo) return `<div class="rs-fc-sep">${labelSep}</div><div class="rs-fc-vazio">Nenhuma votação aberta no momento.</div>`;

let snap; try { snap = await window.db.collection('sugestoes_publicas').where('ciclo_id', '==', cicloAtivo.id).orderBy('votos', 'desc').limit(50).get(); } catch (e) { snap = await window.db.collection('sugestoes_publicas').where('ciclo_id', '==', cicloAtivo.id).limit(50).get(); }
if (snap.empty) return `<div class="rs-fc-sep">${labelSep}</div><div class="rs-fc-vazio">Nenhuma sugestão ativa ainda.</div>`;

const docsArray = snap.docs.map(d => ({ id: d.id, ...d.data() })); docsArray.sort((a, b) => (b.votos || 0) - (a.votos || 0)); const top5 = docsArray.slice(0, 5);
...
sugestoes.forEach((s, i) => {
  ...
  const voteBtn = podeVotar ? `<button class="rs-voto-btn ${jaVotou ? 'votado' : ''}" onclick="window.votarSugestao('${_esc(s.id)}', '${_esc(user.uid)}', '${_esc(cicloAtivo.id)}')">${jaVotou ? '✅ Votado' : '👍 Votar'}</button>` : '';
  const dataEnv = s.criado_em ? new Date(s.criado_em.seconds ? s.criado_em.seconds * 1000 : s.criado_em).toLocaleDateString('pt-BR') : '—';
  html += `<div class="rs-sugestao-card"><div class="rs-sugestao-header"><span class="rs-sugestao-posicao">#${i + 1}</span><span class="rs-sugestao-votos">👍 ${s.votos || 0}</span></div><div class="rs-sugestao-texto">${_esc(s.texto)}</div><div class="rs-sugestao-data">ENVIADA em ${dataEnv}</div>${voteBtn}</div>`;
});
return html;
} catch (err) { console.error('[faleConosco] Erro detalhado ao carregar votação:', err); return `<div class="rs-fc-vazio"><span>⚠️</span>Erro ao carregar votação.</div>`; }
}
...
// ── Boot ──────────────────────────────────────────────────────────────────
function _boot() { if (window._radarUser && window.db) init(); else window.addEventListener('radarUserReady', () => setTimeout(init, 500), { once: true }); }
