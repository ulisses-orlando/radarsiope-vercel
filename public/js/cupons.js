/* cupons.js - CRUD compatível com admin.html
   ...
   Campos adicionados (suporte a cupons multi-usuário / associações):
   - plano_id   : ID do plano ao qual o cupom é restrito
   - ciclo_cupom: ciclo em meses restrito (1, 3, 6 ou 12); vazio = qualquer ciclo
*/
...
async function abrirModalCupom(id, editar = false) {
  ...
  let dados = { codigo: '', tipo: 'percentual', valor: '', status: 'ativo', expira_em: null, max_usos: 0, plano_id: '', ciclo_cupom: '' };
  if (editar) {
    try {
  ...
  document.getElementById('modal-edit-save').onclick = async () => {
    try {
      ...
      // 3. plano_id e ciclo_cupom já foram coletados pelo loop (têm data-field-name)
