...
async function carregarFeatures() {
  const now = Date.now();
  if (featuresCache && (now - featuresCacheTimestamp) < CACHE_DURATION) return featuresCache;
  try {
    const snap = await db.collection('features').orderBy('ordem', 'asc').orderBy('nome', 'asc').get();
    featuresCache = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
...
async function excluirFeature(id) {
  try {
    await db.collection('features').doc(id).update({ ativo: false, atualizado_em: new Date() });
    featuresCache = null;
    if (typeof window !== 'undefined') window.featuresListCache = null; // 🔹 Limpa cache global
  } catch (err) {
...
function renderCampoFeature(feature, valor) {
  ...
  const label = `${feature.icone || '⚙️'} ${feature.nome}`;
  if (feature.tipo === 'boolean') {
    return `<div style="display:flex;align-items:center;justify-content:space-between"><label for="${id}" style="font-size:13px;cursor:pointer">${escapeHtml(label)}</label><input type="checkbox" id="${id}" ${valor ? 'checked' : ''} style="width:16px;height:16px;cursor:pointer"></div>`;
  }
...
function coletarValoresFeatures(features) {
  ...
  features.forEach(feature => {
    ...
    if (!el) return;
    if (feature.tipo === 'boolean') valores[feature.id] = el.checked;
    else if (feature.tipo === 'number') valores[feature.id] = Number(el.value) || 0;
    else if (feature.tipo === 'text') valores[feature.id] = el.value.trim();
  });
  return valores;
}
...
function formatarFeaturesBadges(features, featuresList) {
  ...
  featuresList.forEach(feature => {
    ...
    if (!valor) return;
    if (feature.tipo === 'boolean' && valor === true) ativos.push(feature.icone);
    else if (feature.tipo === 'number' && valor > 0) ativos.push(`${feature.icone}${valor}${feature.unidade || ''}`);
    else if (feature.tipo === 'text' && valor.trim()) ativos.push(feature.icone);
  });
...
async function abrirModalFeatures() {
  ...
  body.innerHTML = '<div style="padding:20px;text-align:center">Carregando features...</div>';
  ...
  try {
    ...
    body.innerHTML = '<div style="padding:20px;text-align:center;color:#b00020">Erro ao carregar features</div>';
  }
}
...
async function abrirModalFeature(id = null) {
  ...
  let d = { nome: '', descricao: '', tipo: 'boolean', unidade: '', icone: '⚙️', ordem: 99, ativo: true };
  if (id) {
    try {
      const doc = await db.collection('features').doc(id).get();
      if (doc.exists) d = { ...d, ...doc.data() };
    } catch (err) {
  ...
  body.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:12px">
      <div>
        ...
        ${id ? '<div style="font-size:11px;color:#888;margin-top:2px">ID não pode ser alterado após criação</div>' : '<div style="font-size:11px;color:#888;margin-top:2px">Use apenas letras minúsculas, números e underscore (_)</div>'}
      </div>
      <div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px">
        <div>
          ...
          <input id="feat-icone" type="text" value="${d.icone || '⚙️'}" style="width:100%;padding:8px;border:1px solid #ccc;border-radius:4px" placeholder="Cole um emoji aqui (ex: 🚀)" onpaste="this.value = (event.clipboardData || window.clipboardData).getData('text/plain');">
        </div>
        <div>
  ...
  if (saveBtn) saveBtn.onclick = () => _salvarFeatureModal(id);
  openModal && openModal('modal-edit-overlay');
}

function confirmarExclusaoFeature(id, nome) {
  abrirConfirmacao && abrirConfirmacao(`Excluir feature "${nome}"?\n\nEsta ação marcará a feature como inativa. Planos existentes não serão afetados.`, () => excluirFeature(id).then(() => {
...
async function _salvarFeatureModal(id) {
  ...
  if (!customId && !id) { mostrarMensagem('ID da feature é obrigatório'); return; }
  if (!id && !/^[a-z0-9_]+$/.test(customId)) { mostrarMensagem('ID deve conter apenas letras minúsculas, números e underscore (_)'); return; }
  if (!data.nome) { mostrarMensagem('Nome da feature é obrigatório'); return; }
