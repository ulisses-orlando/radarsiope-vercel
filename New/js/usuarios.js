/* ========================
   ...
   ======================== */
let planMap = {}, planList = [], planosLoaded = false;
  ...
  snap.forEach(doc => {
    ...
    tr.innerHTML = `
      ...
      <td>
        ...
        <span class="icon-btn" title="Logs" onclick="abrirSubColecao('usuarios','${doc.id}','logs_acesso','${(d.nome||'sem-nome').replace(/'/g,"\\'")}')">📜</span>
        <span class="icon-btn" title="Assinaturas" onclick="abrirSubColecao('usuarios','${doc.id}','assinaturas','${(d.nome||'sem-nome').replace(/'/g,"\\'")}')">📝</span>
        <span class="icon-btn" title="Preferências" onclick="abrirSubColecao('usuarios','${doc.id}','preferencias_newsletter','${(d.nome||'sem-nome').replace(/'/g,"\\'")}')">⭐</span>
...
function abrirModalCriarUsuario(){
  ...
  document.getElementById('modal-edit-save').onclick = async ()=>{
    ...
    body.querySelectorAll('[data-field-name]').forEach(el=>{
      let v = el.value;
      if(el.tagName==='SELECT' && el.dataset.fieldName==='ativo') v = (v==='true');
...
async function initUsuarios(){ await loadPlanos(); await listarUsuarios(); }
initUsuarios();
